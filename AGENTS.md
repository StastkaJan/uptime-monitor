...
- Use Docker Compose as the default development and test workflow. Keep the Go image version aligned with the module and CI; preserve the SQLite named volume.
...
- Bind native runs to loopback by default. In Compose, listen on all container interfaces but publish only to host loopback. Follow the public-demo constraints in docs/architecture.md before exposing the service.
